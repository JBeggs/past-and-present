...
interface ProductsListWhatsAppShareDialogProps {
  open: boolean
  onClose: () => void
...
export default function ProductsListWhatsAppShareDialog({
  open,
  onClose,
  initialMessage,
  shareImageUrls,
}: ProductsListWhatsAppShareDialogProps) {
  ...
  const handleShare = async () => {
    ...
    setSharing(true)
    try {
      const result = await shareProductsListOnWhatsApp(message, shareImageUrls)
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="products-list-whatsapp-share-title"
      onClick={(event) => {
    ...
    >
      <div className="w-full max-w-xl rounded-2xl border border-[#e8e4df] bg-white shadow-2xl">
        <div className="p-5">
          <h2 id="products-list-whatsapp-share-title" className="text-xl font-bold font-playfair text-text mb-2">
            Share this product list
          </h2>
          <p className="text-sm text-text-muted mb-4">
            Edit your message, then share. On mobile, pick WhatsApp — up to four product photos from this shelf are attached when your browser allows it.
          </p>

          <label htmlFor="products-list-whatsapp-message" className="mb-2 block text-xs font-medium uppercase tracking-wide text-[#8a837a]">
            Message text
          </label>
          <textarea
            id="products-list-whatsapp-message"
            rows={8}
            value={messageText}
          ...
          <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button type="button" onClick={onClose} className="btn btn-secondary" disabled={sharing}>
              Cancel
            </button>
            <button
              type="button"
