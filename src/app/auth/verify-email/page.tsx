    ...
    <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-xl border border-vintage-primary/10 text-center space-y-4">
      {!token ? (
        <>
          ...
          {emailHint ? (
            <button type="button" onClick={() => void resend()} disabled={resendBusy} className="btn btn-secondary">
              {resendBusy ? 'Sending…' : 'Resend verification email'}
            </button>
          ) : null}
          <Link href="/login" className="btn btn-primary block w-full">
            Back to sign in
          </Link>
        </>
      ) : (
        <>
          ...
          {status === 'error' ? (
            <>
              ...
              <Link href="/login" className="btn btn-secondary block w-full">
                Sign in
              </Link>
            </>
          ) : null}
...
export default function VerifyEmailPage() {
  return (
    <div className="min-h-screen bg-vintage-background flex items-center justify-center py-12 px-4">
      <Suspense
        fallback={<div className="bg-white p-8 rounded-xl text-text-muted">Loading…</div>}
      >
        <VerifyEmailInner />
      </Suspense>
    </div>
